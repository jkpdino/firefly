use crate::{ir::{code::{BasicBlockId, Function, Local}, ty::Ty, value::{Immediate, Place}, VirContext}, util::{Id, UniqueId}};
...
impl<'a> Builder<'a> {
    ...
    pub fn append_basic_block(&mut self) -> BasicBlockId {
        ...
        self.context_mut().create_basic_block(func_id)
    }

    /// Builds a local variable in the selected function
    pub fn build_local(&mut self, ty: &Ty) -> Id<Local> {
        let func_id = self.current_func_id();
