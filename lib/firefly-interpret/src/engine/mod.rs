...
impl<'a> ExecutionEngine<'a> {
    ...
    fn eval_unary(&mut self, value: &Immediate, frame: &mut StackFrame, op: &crate::ir::value::UnaryIntrinsic) -> InnerValue {
        let operand = self.eval_immediate(value, frame);

        match (operand.as_ref(), op) {
            (InnerValue::Integer(i), crate::ir::value::UnaryIntrinsic::BitNot) => {
                return InnerValue::Integer(!i);
            }

            (InnerValue::Boolean(b), crate::ir::value::UnaryIntrinsic::Not) => {
                return InnerValue::Boolean(!b);
            }

            (InnerValue::String(s), crate::ir::value::UnaryIntrinsic::Len) => {
                return InnerValue::Integer(s.len() as u64);
            }

            (InnerValue::Integer(i), crate::ir::value::UnaryIntrinsic::Format) => {
                return InnerValue::String(i.to_string());
            }

            (InnerValue::String(s), crate::ir::value::UnaryIntrinsic::Parse) => {
                return InnerValue::Integer(s.parse().unwrap());
            }

            (InnerValue::Boolean(b), crate::ir::value::UnaryIntrinsic::Format) => {
                return InnerValue::String(b.to_string());
            }

            (InnerValue::String(s), crate::ir::value::UnaryIntrinsic::Print) => {
                println!("{s}");
